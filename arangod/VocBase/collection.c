...
void TRI_InitCollectionInfo (TRI_vocbase_t* vocbase,
  ...
  parameter->_isTransactional = false;
...
void TRI_CopyCollectionInfo (TRI_col_info_t* dst, const TRI_col_info_t* const src) {
  ...
  dst->_isTransactional = src->_isTransactional;
...
int TRI_LoadCollectionInfo (char const* path, TRI_col_info_t* parameter) {
  ...
  for (i = 0;  i < n;  i += 2) {
    ...
    if (key->_type == TRI_JSON_STRING && value->_type == TRI_JSON_NUMBER) {
      ...
      else if (TRI_EqualString(key->_value._string.data, "isTransactional")) {
        parameter->_isTransactional = value->_value._boolean;
      }
...
int TRI_SaveCollectionInfo (char const* path, const TRI_col_info_t* const info) {
  ...
  TRI_Insert3ArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "isTransactional", TRI_CreateBooleanJson(TRI_UNKNOWN_MEM_ZONE, info->_isTransactional));
...
int TRI_UpdateCollectionInfo (TRI_vocbase_t* vocbase, 
  ...
  if (parameter != 0) {
    ...
    // the following collection properties are intentionally not updated because updating
